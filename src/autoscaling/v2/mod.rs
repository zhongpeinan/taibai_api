//! Kubernetes Autoscaling v2 API types
//!
//! This module contains types from the Kubernetes autoscaling/v2 API group.
//!
//! Source: https://github.com/kubernetes/api/blob/master/autoscaling/v2/types.go
...
impl ApplyDefault for HorizontalPodAutoscaler {
    fn apply_default(&mut self) {
        if self.type_meta.api_version.is_empty() {
            self.type_meta.api_version = "autoscaling/v2".to_string();
        }
        if self.type_meta.kind.is_empty() {
            self.type_meta.kind = "HorizontalPodAutoscaler".to_string();
        }
        crate::autoscaling::v2::defaults::set_defaults_horizontal_pod_autoscaler(self);
    }
}
