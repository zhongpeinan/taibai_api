//! Conversions between autoscaling v2 and internal types
//!
//! Based on k8s.io/kubernetes/pkg/apis/autoscaling/v2/conversion.go
...
impl FromInternal<internal::HorizontalPodAutoscaler> for HorizontalPodAutoscaler {
    fn from_internal(value: internal::HorizontalPodAutoscaler) -> Self {
        let mut result = Self {
            ...
            spec: value.spec.map(convert_spec_from_internal),
            status: value.status.map(convert_status_from_internal),
        };
