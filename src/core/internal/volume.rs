...
mod tests {
    ...
    #[test]
    fn test_volume_mount_default() {
        let mount = VolumeMount::default();
        assert!(mount.name.is_empty());
        assert!(!mount.read_only);
        assert!(mount.recursive_read_only.is_none());
        assert!(mount.mount_path.is_empty());
    }

    #[test]
    fn test_volume_mount_with_fields() {
        let mount = VolumeMount {
            name: "data".to_string(),
            read_only: true,
            recursive_read_only: Some(RecursiveReadOnlyMode::Enabled),
        ...
        assert!(mount.read_only);
        assert_eq!(
            mount.recursive_read_only,
            Some(RecursiveReadOnlyMode::Enabled)
        );
        assert_eq!(mount.mount_path, "/data");
    }

    #[test]
    fn test_volume_mount_serialize() {
        let mount = VolumeMount {
        ...
        assert!(json.contains("\"mountPath\":\"/config\""));
        assert!(json.contains("\"readOnly\":false"));
        // None/empty fields should be omitted
        assert!(!json.contains("\"recursiveReadOnly\""));
        assert!(!json.contains("\"subPath\""));
        assert!(!json.contains("\"mountPropagation\""));
        assert!(!json.contains("\"subPathExpr\""));
    }

    #[test]
    fn test_volume_mount_with_all_fields_serialize() {
        let mount = VolumeMount {
            name: "storage".to_string(),
            read_only: true,
            recursive_read_only: Some(RecursiveReadOnlyMode::IfPossible),
            mount_path: "/mnt/storage".to_string(),
            sub_path: "subdir".to_string(),
            mount_propagation: Some(MountPropagationMode::HostToContainer),
            sub_path_expr: "$(SUBPATH_EXPR)".to_string(),
        };

        let json = serde_json::to_string(&mount).unwrap();
        assert!(json.contains("\"name\":\"storage\""));
        assert!(json.contains("\"readOnly\":true"));
        assert!(json.contains("\"recursiveReadOnly\":\"IfPossible\""));
        assert!(json.contains("\"subPath\":\"subdir\""));
        assert!(json.contains("\"mountPropagation\":\"HostToContainer\""));
        assert!(json.contains("\"subPathExpr\":\"$(SUBPATH_EXPR)\""));
    }

    #[test]
    fn test_volume_mount_deserialize() {
        let json = r#"{"name":"data","readOnly":true,"mountPath":"/data"}"#;
    ...
    #[test]
    fn test_volume_mount_round_trip() {
        let original = VolumeMount {
            name: "logs".to_string(),
            read_only: false,
            recursive_read_only: Some(RecursiveReadOnlyMode::Disabled),
    ...
    #[test]
    fn test_volume_device_default() {
        let device = VolumeDevice::default();
        assert!(device.name.is_empty());
        assert!(device.device_path.is_empty());
    }

    #[test]
    fn test_volume_device_with_fields() {
        let device = VolumeDevice {
            name: "block-device".to_string(),
            device_path: "/dev/xvda".to_string(),
        };

        assert_eq!(device.name, "block-device");
        assert_eq!(device.device_path, "/dev/xvda");
    }

    #[test]
    fn test_volume_device_serialize() {
        let device = VolumeDevice {
            name: "disk".to_string(),
        ...
        assert!(json.contains("\"devicePath\":\"/dev/sda\""));
    }

    #[test]
    fn test_volume_device_deserialize() {
        let json = r#"{"name":"data","devicePath":"/dev/xvdb"}"#;
        let device: VolumeDevice = serde_json::from_str(json).unwrap();

        assert_eq!(device.name, "data");
        assert_eq!(device.device_path, "/dev/xvdb");
    }

    #[test]
    fn test_volume_device_round_trip() {
        let original = VolumeDevice {
            name: "ephemeral".to_string(),
            device_path: "/dev/xvdc".to_string(),
        };
    ...
    #[test]
    fn test_recursive_read_only_mode_serialize() {
        let json = serde_json::to_string(&RecursiveReadOnlyMode::Disabled).unwrap();
        assert_eq!(json, r#""Disabled""#);

        let json = serde_json::to_string(&RecursiveReadOnlyMode::IfPossible).unwrap();
        assert_eq!(json, r#""IfPossible""#);

        let json = serde_json::to_string(&RecursiveReadOnlyMode::Enabled).unwrap();
        assert_eq!(json, r#""Enabled""#);
    }

    #[test]
    fn test_recursive_read_only_mode_deserialize() {
        let mode: RecursiveReadOnlyMode = serde_json::from_str(r#""Disabled""#).unwrap();
        assert_eq!(mode, RecursiveReadOnlyMode::Disabled);

        let mode: RecursiveReadOnlyMode = serde_json::from_str(r#""Enabled""#).unwrap();
        assert_eq!(mode, RecursiveReadOnlyMode::Enabled);
    }

    #[test]
    fn test_mount_propagation_mode_serialize() {
        let json = serde_json::to_string(&MountPropagationMode::None).unwrap();
        assert_eq!(json, r#""None""#);

        let json = serde_json::to_string(&MountPropagationMode::HostToContainer).unwrap();
        assert_eq!(json, r#""HostToContainer""#);

        let json = serde_json::to_string(&MountPropagationMode::Bidirectional).unwrap();
        assert_eq!(json, r#""Bidirectional""#);
    }

    #[test]
    fn test_mount_propagation_mode_deserialize() {
        let mode: MountPropagationMode = serde_json::from_str(r#""HostToContainer""#).unwrap();
        assert_eq!(mode, MountPropagationMode::HostToContainer);

        let mode: MountPropagationMode = serde_json::from_str(r#""Bidirectional""#).unwrap();
        assert_eq!(mode, MountPropagationMode::Bidirectional);
    }
}
