...
mod tests {
    ...
    #[test]
    fn test_persistent_volume_list_default() {
        let list = PersistentVolumeList::default();
        assert!(list.metadata.is_none());
        assert!(list.items.is_empty());
    }

    #[test]
    fn test_iscsi_pv_source_with_secret_ref() {
        let source = ISCSIPersistentVolumeSource {
            target_portal: "127.0.0.1:3260".to_string(),
            iqn: "iqn.2001-04.com.example:storage".to_string(),
            lun: 0,
            ..Default::default()
        };

        assert_eq!(source.target_portal, "127.0.0.1:3260");
        assert_eq!(source.iqn, "iqn.2001-04.com.example:storage");
        assert_eq!(source.lun, 0);
    }

    #[test]
    fn test_rbd_pv_source_serialize() {
        let source = RBDPersistentVolumeSource {
            ceph_monitors: vec!["10.16.154.77:6789".to_string()],
            rbd_image: "foo".to_string(),
            ..Default::default()
        };

        let json = serde_json::to_string(&source).unwrap();
        assert!(json.contains("\"cephMonitors\""));
        assert!(json.contains("\"rbdImage\":\"foo\""));
    }

    #[test]
    fn test_csi_pv_source_with_multiple_secrets() {
        let source = CSIPersistentVolumeSource {
            ...
            controller_publish_secret_ref: Some(SecretReference {
                name: "csi-controller-secret".to_string(),
                namespace: "default".to_string(),
            }),
            node_stage_secret_ref: Some(SecretReference {
                name: "csi-node-secret".to_string(),
                namespace: "kube-system".to_string(),
            }),
            ..Default::default()
        };

        assert_eq!(source.driver, "csi.example.com");
        assert!(source.controller_publish_secret_ref.is_some());
        assert!(source.node_stage_secret_ref.is_some());
    }
}
