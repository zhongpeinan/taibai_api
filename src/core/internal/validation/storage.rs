//! PersistentVolume and PersistentVolumeClaim validation for Kubernetes core internal API
//!
...
pub fn validate_persistent_volume_spec(spec: &PersistentVolumeSpec, path: &Path) -> ErrorList {
    ...
    if !spec.persistent_volume_reclaim_policy.is_empty()
        && !SUPPORTED_RECLAIM_POLICIES.contains(spec.persistent_volume_reclaim_policy.as_str())
    {
        let valid: Vec<&str> = SUPPORTED_RECLAIM_POLICIES.iter().copied().collect();
        all_errs.push(not_supported(
            &path.child("persistentVolumeReclaimPolicy"),
            BadValue::String(spec.persistent_volume_reclaim_policy.clone()),
            &valid,
        ));
    }
    ...
    if !spec.storage_class_name.is_empty() {
        for err_msg in is_dns1123_subdomain(&spec.storage_class_name) {
            all_errs.push(invalid(
                &path.child("storageClassName"),
                BadValue::String(spec.storage_class_name.clone()),
                &err_msg,
            ));
        }
    }
    ...
    all_errs.extend(validate_persistent_volume_source(
        &spec.persistent_volume_source,
        path,
    ));
    ...
    if let Some(ref host_path) = spec.persistent_volume_source.host_path {
        if spec.persistent_volume_reclaim_policy == persistent_volume_reclaim_policy::RECYCLE
    ...
    if spec.persistent_volume_source.local.is_some() && !node_affinity_specified {
        all_errs.push(required(
            &path.child("nodeAffinity"),
            "Local volume requires node affinity",
        ));
...
pub fn validate_persistent_volume_claim_spec(
    ...
    if let (Some(data_source), Some(data_source_ref)) = (&spec.data_source, &spec.data_source_ref) {
...
pub fn validate_persistent_volume_claim_update(
    ...
    if let (Some(new_spec), Some(old_spec)) = (&new_pvc.spec, &old_pvc.spec) {
        ...
        if !old_spec.volume_name.is_empty() && new_spec.volume_name != old_spec.volume_name {
            all_errs.push(forbidden(
...
fn validate_host_path_pv_source(
    host_path: &internal_pv::HostPathVolumeSource,
    path: &Path,
) -> ErrorList {
    let mut all_errs = ErrorList::new();
...
fn validate_nfs_pv_source(nfs: &internal_pv::NFSVolumeSource, path: &Path) -> ErrorList {
    let mut all_errs = ErrorList::new();
...
fn validate_iscsi_pv_source(
    iscsi: &internal_pv::ISCSIPersistentVolumeSource,
    path: &Path,
) -> ErrorList {
    let mut all_errs = ErrorList::new();
...
fn validate_fc_pv_source(fc: &internal_pv::FCVolumeSource, path: &Path) -> ErrorList {
    let mut all_errs = ErrorList::new();
...
fn validate_local_volume_source(
    local: &crate::core::internal::LocalVolumeSource,
    path: &Path,
) -> ErrorList {
    let mut all_errs = ErrorList::new();
...
fn validate_csi_pv_source(csi: &internal_pv::CSIPersistentVolumeSource, path: &Path) -> ErrorList {
    let mut all_errs = ErrorList::new();
...
fn validate_data_source_ref(data_source_ref: &TypedLocalObjectReference, path: &Path) -> ErrorList {
    let mut all_errs = ErrorList::new();
...
fn host_path_is_root(host_path: &internal_pv::HostPathVolumeSource) -> bool {
    host_path.path == "/"
}
