...
use crate::core::v1::event::{Event, event_type};
...
pub fn validate_event_create(event: &Event, request_version: EventRequestVersion) -> ErrorList {
    ...
    let event_type_value = event.type_.as_deref().unwrap_or("");
    ...
    if event.source.as_ref().is_some_and(|source| {
        !source.component.as_deref().unwrap_or("").is_empty()
            || !source.host.as_deref().unwrap_or("").is_empty()
    }) {
        all_errs.push(invalid(
            &Path::new("source"),
            BadValue::String(String::new()),
...
pub fn validate_event_update(
    ...
    let new_meta = new_event.metadata.clone().unwrap_or_default();
    let old_meta = old_event.metadata.clone().unwrap_or_default();
...
fn validate_v1_event_series(event: &Event) -> ErrorList {
    ...
    if let Some(series) = &event.series {
        if series.count.unwrap_or(0) < 2 {
            all_errs.push(invalid(
                &Path::new("series").child("count"),
                BadValue::Int(series.count.unwrap_or(0) as i64),
...
fn legacy_validate_event(event: &Event, request_version: EventRequestVersion) -> ErrorList {
    ...
    if event_time_is_zero {
        ...
        let reporting_controller = event.reporting_controller.as_deref().unwrap_or("");
        ...
        let reporting_instance = event.reporting_instance.as_deref().unwrap_or("");
...
mod tests {
    use super::*;
    use crate::common::time::MicroTime;
    use crate::core::v1::event::{Event, EventSeries, EventSource};
