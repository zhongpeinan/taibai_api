...
mod tests {
    ...
    #[test]
    fn test_validate_resource_claim_request_requires_type() {
        let claim = internal::ResourceClaim {
            type_meta: Default::default(),
            metadata: ObjectMeta {
                name: Some("test-claim".to_string()),
        ...
        assert!(
            errors
                .errors
                .iter()
                .any(|e| e.field.contains("spec.devices.requests[0]")),
            "expected request type error, got {errors:?}"
        );
    }

    #[test]
    fn test_validate_resource_claim_update_spec_immutable() {
        let old = internal::ResourceClaim {
            type_meta: Default::default(),
            metadata: ObjectMeta {
                name: Some("test-claim".to_string()),
                namespace: Some("default".to_string()),
        ...
        new.spec.devices.requests.push(DeviceRequest {
            name: "req1".to_string(),
            exactly: Some(ExactDeviceRequest {
                device_class_name: "class-a".to_string(),
                allocation_mode: DeviceAllocationMode::ExactCount,
                count: 1,
                ..Default::default()
            }),
        ...
        let errors = validate_resource_claim_update(&new, &old);
        assert!(
            errors.errors.iter().any(|e| e.field == "spec"),
            "expected spec immutable error, got {errors:?}"
        );
    }

    #[test]
    fn test_validate_resource_slice_requires_node_selection() {
        let mut slice = internal::ResourceSlice {
            type_meta: Default::default(),
            metadata: ObjectMeta {
                name: Some("test-slice".to_string()),
                ..Default::default()
        ...
        assert!(
            errors.errors.iter().any(|e| e.field == "spec"),
            "expected node selection error, got {errors:?}"
        );

        slice.spec.node_name = Some("node-1".to_string());
        slice.spec.all_nodes = Some(true);
        let errors = validate_resource_slice(&slice);
        assert!(
            errors.errors.iter().any(|e| e.field == "spec"),
    ...
    #[test]
    fn test_validate_device_class_selector_requires_cel() {
        let class = DeviceClass {
            type_meta: Default::default(),
            metadata: ObjectMeta {
                name: Some("test-class".to_string()),
            ...
            spec: DeviceClassSpec {
                selectors: vec![DeviceClassSelector { cel: None }],
                config: vec![],
                extended_resource_name: None,
            },
        };
        ...
        assert!(
            errors
                .errors
                .iter()
                .any(|e| e.field.contains("spec.selectors[0].cel")),
            "expected selector cel error, got {errors:?}"
        );
    }

    #[test]
    fn test_validate_device_class_config_requires_opaque() {
        let class = DeviceClass {
            type_meta: Default::default(),
            metadata: ObjectMeta {
                name: Some("test-class".to_string()),
            ...
            spec: DeviceClassSpec {
                selectors: vec![],
                config: vec![DeviceClassConfiguration { opaque: None }],
                extended_resource_name: None,
            },
        };
