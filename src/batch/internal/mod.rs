...
mod tests {
    ...
    // CompletionMode tests
    #[test]
    fn test_completion_mode_serialize() {
        assert_eq!(
            serde_json::to_string(&CompletionMode::NonIndexed).unwrap(),
            r#""NonIndexed""#
        );
    }

    #[test]
    fn test_completion_mode_deserialize() {
        assert_eq!(
            serde_json::from_str::<CompletionMode>(r#""Indexed""#).unwrap(),
            CompletionMode::Indexed
        );
    }

    #[test]
    fn test_completion_mode_constants() {
        assert_eq!(completion_mode::NON_INDEXED, "NonIndexed");
        assert_eq!(completion_mode::INDEXED, "Indexed");
    }

    // PodFailurePolicyAction tests
    #[test]
    fn test_pod_failure_policy_action_serialize() {
        assert_eq!(
            serde_json::to_string(&PodFailurePolicyAction::FailJob).unwrap(),
            r#""FailJob""#
        );
    }

    #[test]
    fn test_pod_failure_policy_action_deserialize() {
        assert_eq!(
            serde_json::from_str::<PodFailurePolicyAction>(r#""FailIndex""#).unwrap(),
            PodFailurePolicyAction::FailIndex
        );
    }

    #[test]
    fn test_pod_failure_policy_action_constants() {
        assert_eq!(pod_failure_policy_action::FAIL_JOB, "FailJob");
        assert_eq!(pod_failure_policy_action::FAIL_INDEX, "FailIndex");
        assert_eq!(pod_failure_policy_action::IGNORE, "Ignore");
        assert_eq!(pod_failure_policy_action::COUNT, "Count");
    }

    // PodFailurePolicyOnExitCodesOperator tests
    #[test]
    fn test_pod_failure_policy_on_exit_codes_operator_serialize() {
        assert_eq!(
            serde_json::to_string(&PodFailurePolicyOnExitCodesOperator::In).unwrap(),
            r#""In""#
        );
    }

    #[test]
    fn test_pod_failure_policy_on_exit_codes_operator_deserialize() {
        assert_eq!(
            serde_json::from_str::<PodFailurePolicyOnExitCodesOperator>(r#""NotIn""#).unwrap(),
            PodFailurePolicyOnExitCodesOperator::NotIn
        );
    }

    // PodReplacementPolicy tests
    #[test]
    fn test_pod_replacement_policy_serialize() {
        assert_eq!(
            serde_json::to_string(&PodReplacementPolicy::TerminatingOrFailed).unwrap(),
            r#""TerminatingOrFailed""#
        );
    }

    #[test]
    fn test_pod_replacement_policy_deserialize() {
        assert_eq!(
            serde_json::from_str::<PodReplacementPolicy>(r#""Failed""#).unwrap(),
            PodReplacementPolicy::Failed
        );
    }

    // JobConditionType tests
    #[test]
    fn test_job_condition_type_serialize() {
        assert_eq!(
            serde_json::to_string(&JobConditionType::Complete).unwrap(),
            r#""Complete""#
        );
    }

    #[test]
    fn test_job_condition_type_deserialize() {
        assert_eq!(
            serde_json::from_str::<JobConditionType>(r#""Failed""#).unwrap(),
            JobConditionType::Failed
        );
    }

    #[test]
    fn test_job_condition_type_constants() {
        assert_eq!(job_condition_type::SUSPENDED, "Suspended");
        assert_eq!(job_condition_type::COMPLETE, "Complete");
        assert_eq!(job_condition_type::FAILED, "Failed");
    }
    ...
    fn test_label_constants() {
        assert_eq!(
            label_prefix::JOB_TRACKING_FINALIZER,
            "batch.kubernetes.io/job-tracking"
        );
        assert_eq!(label_prefix::JOB_NAME_LABEL, "batch.kubernetes.io/job-name");
        assert_eq!(
            controller_name::JOB_CONTROLLER_NAME,
