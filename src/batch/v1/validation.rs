...
mod tests {
    ...
    #[test]
    fn test_validate_job_missing_selector_with_manual_selector_true() {
        // When manualSelector is true, selector MUST be specified
        let job = Job {
            type_meta: TypeMeta::default(),
            metadata: Some(ObjectMeta {
                name: Some("test-job".to_string()),
            ...
            spec: Some(JobSpec {
                selector: None,
                manual_selector: Some(true), // Requires selector to be specified
                template: PodTemplateSpec::default(),
                ..Default::default()
            }),
            status: None,
        ...
        assert!(
            ...
            "Expected error for missing selector when manualSelector is true"
        );
    }

    #[test]
    fn test_validate_job_missing_selector_auto_generated() {
        // When manualSelector is false/nil (default), selector is auto-generated by API server
        // so it's OK if it's not specified
        let job = Job {
            type_meta: TypeMeta::default(),
            metadata: Some(ObjectMeta {
                name: Some("test-job".to_string()),
            ...
            spec: Some(JobSpec {
                selector: None,
                manual_selector: None, // Default: selector will be auto-generated
                template: PodTemplateSpec::default(),
                ..Default::default()
            }),
            status: None,
        ...
        let errors = validate_job(&job);
        // No selector error expected when manualSelector is not true
        assert!(
            !errors
                .errors
                .iter()
                .any(|e| e.field.to_string().contains("selector")),
            "Should not have selector error when manualSelector is not true"
        );
    }
