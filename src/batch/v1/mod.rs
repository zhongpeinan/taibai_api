...
mod tests {
    ...
    #[test]
    fn test_job_spec_default() {
        let spec = JobSpec::default();
        assert!(spec.parallelism.is_none());
        assert!(spec.completions.is_none());
    }
    ...
    fn test_concurrency_policy_deserialize() {
        assert_eq!(
            serde_json::from_str::<ConcurrencyPolicy>(r#""Forbid""#).unwrap(),
    ...
    #[test]
    fn test_uncounted_terminated_pods_default() {
        let pods = UncountedTerminatedPods::default();
        assert!(pods.succeeded.is_empty());
        assert!(pods.failed.is_empty());
    }

    #[test]
    fn test_success_policy_rule_default() {
        let rule = SuccessPolicyRule::default();
        assert!(rule.succeeded_indexes.is_none());
        assert!(rule.succeeded_count.is_none());
    }

    #[test]
    fn test_pod_failure_policy_default() {
        let policy = PodFailurePolicy::default();
        assert!(policy.rules.is_empty());
    }

    #[test]
    fn test_cron_job_default() {
        let cronjob = CronJob::default();
        assert!(cronjob.metadata.is_none());
        assert!(cronjob.spec.is_none());
        assert!(cronjob.status.is_none());
    }
    ...
    #[test]
    fn test_cron_job_spec_default() {
        let spec = CronJobSpec::default();
        assert!(spec.schedule.is_empty());
        assert_eq!(spec.concurrency_policy, ConcurrencyPolicy::Allow);
    }

    #[test]
    fn test_cron_job_status_default() {
    ...
    #[test]
    fn test_job_template_spec_default() {
        let template = JobTemplateSpec::default();
    ...
    fn test_constants() {
        ...
        assert_eq!(job_reason::POD_FAILURE_POLICY, "PodFailurePolicy");
    }

    #[test]
    fn test_concurrency_policy_constants() {
        assert_eq!(concurrency_policy::ALLOW, "Allow");
        assert_eq!(concurrency_policy::FORBID, "Forbid");
        assert_eq!(concurrency_policy::REPLACE, "Replace");
    }
}
