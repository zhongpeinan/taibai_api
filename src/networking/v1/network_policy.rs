...
mod tests {
    ...
    #[test]
    fn test_network_policy_apply_default() {
        let mut np = NetworkPolicy {
            type_meta: TypeMeta::default(),
            ..Default::default()
        };
        np.apply_default();
        assert_eq!(np.type_meta.api_version, "networking.k8s.io/v1");
        assert_eq!(np.type_meta.kind, "NetworkPolicy");
    }

    #[test]
    fn test_network_policy_list_default() {
        let list = NetworkPolicyList::default();
        assert!(list.items.is_empty());
    }

    #[test]
    fn test_policy_type_constants() {
        assert_eq!(policy_type::INGRESS, "Ingress");
        assert_eq!(policy_type::EGRESS, "Egress");
    }
}
