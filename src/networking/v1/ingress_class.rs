...
mod tests {
    ...
    #[test]
    fn test_ingress_class_default() {
        let ic = IngressClass::default();
        assert!(ic.metadata.is_none());
        assert!(ic.spec.controller.is_empty());
    }

    #[test]
    fn test_ingress_class_serialize() {
        let ic = IngressClass {
            type_meta: TypeMeta {
        ...
        assert!(json.contains(r#""kind":"IngressClass""#));
        assert!(json.contains("example.com/ingress-controller"));
    }

    #[test]
    fn test_ingress_class_apply_default() {
        let mut ic = IngressClass {
            type_meta: TypeMeta::default(),
            ..Default::default()
        };
        ic.apply_default();
        assert_eq!(ic.type_meta.api_version, "networking.k8s.io/v1");
        assert_eq!(ic.type_meta.kind, "IngressClass");
    }

    #[test]
    fn test_ingress_class_list_default() {
        let list = IngressClassList::default();
